...
function getShadowEffectProperties(params) {
  const [container] = Object.values(params)[0];
  
  return {
    // For some reason in XD, if you enter blur = 4, it divides it by 2 in the output file
    blur: container.r * 2,
    x: container.dx,
    y: container.dy,
    spread: null,
...
function mapShadow(artboard) {
  const [shadowParams] = artboard.value.style.filters;
