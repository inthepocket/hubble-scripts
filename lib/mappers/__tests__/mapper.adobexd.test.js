...
describe('Adobe XD mapper test suite', () => {
  ...
  describe('-> #formatTextStyles:', () => {
    it('Given Adobe XD formatted array of JSON textstyles, should transform to a nicely parsed array', () => {
      expect(FIXTURES.textStyles.map(mappers.textStyles)).toMatchSnapshot();
    });
  });

  describe('-> #formatGradients:', () => {
    it('Given Adobe XD formatted array of JSON gradients, should transform to a nicely parsed array', () => {
      expect(FIXTURES.gradients.map(mappers.gradients)).toMatchSnapshot();
    });
  });

  describe('-> #formatShadows', () => {
    it('Given Adobe XD formatted array of JSON shadows, should transform to a nicely parsed array', () => {
      expect(FIXTURES.shadows.map(mappers.shadows)).toMatchSnapshot();
    });
  });

  describe('-> #formatBorders:', () => {
    it('Given Adobe XD formatted array of JSON borders, should transform to a nicely parsed array', () => {
      expect(FIXTURES.borders.map(mappers.borders)).toMatchSnapshot();
    });
  });

  describe('-> #formatBlurs:', () => {
    it('Given Adobe XD formatted array of JSON blurs, should transform to a nicely parsed array', () => {
