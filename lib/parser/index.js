...
const { convertAdobeXDToZip, getArtboardFilesFromDir } = require('../adobexd');
...
const parseTokens = async (args, flags) => {
  ...
  if (isSketch(file)) {
    ...
    try {
      ...
      convertAdobeXDToZip(file, outputDir);

      const manifest = JSON.parse(await readFile(path.join(outputDir, 'manifest')));
      const artboardFiles = await getArtboardFilesFromDir(outputDir);

      console.log(manifest.name);
      console.log(artboardFiles[0])
