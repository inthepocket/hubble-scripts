...
const isEmptyObject = obj => isObject(obj) && Object.entries(obj).length < 1;
...
module.exports = {
  ...
  /**
   * Checks is a given object conains a property
   * @param {object} object
   * @param {string} prop
   * @return {boolean}
   */
  ...
  normalizeColorValues: colors => {
    const newValues = Object.values(colors).map(value => value / 255);
    const [ red, green, blue ] = newValues;
  ...
  unzipFile: (filePath, outputDir) => new Promise((resolve, reject) => {
