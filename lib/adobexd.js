...
async function parseXDFile(xdFilePath, outputDir) {
  ...
  colors = colors.map(color => ({ name: color.name, value: color.value.group.children[0].style.fill.color.value }));
  blurs = blurs.map(blur => ({ name: blur.name, value: blur.value })); 
  borders = borders.map(border => ({ name: border.name, value: border.value.style.stroke }));
  gradients = gradients.map(gradient => ({ name: gradient.name, value: gradient.value.style.fill.gradient }));
  textStyles = textStyles.map(textStyle => ({ name: textStyle.name, value: textStyle.value.meta.ux.rangedStyles[0] }));

  await removeDirectory(outputDir);

  return { colors, blurs, borders, gradients, textStyles, groupedTokenReferences };
}
