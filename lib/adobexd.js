...
function filterReferences(references, filter) {
  const filteredElements = references.filter(element => {
    if (filter === 'gradient' && element.name.includes('border')) return false;
...
async function parseXDFile(xdFilePath, outputDir) {
  ...
  const validTokens = tokenReferences.filter(ref => (ref.name.startsWith('primitives') || ref.name.startsWith('textstyle')));

  const tokenArtboards = ['color', 'blur', 'border', 'gradient', 'textstyle', 'shadow'];
  ...
  const matchedTokens = groupedTokenReferences.map((tokenGroup, index) => tokenGroup.elements.map(ref => {
    ...
    const regex = new RegExp(`(?:primitives/|textstyle/)(${tokenArtboards[index]}/)?`, 'gi');
  ...
  const [colorTokens, blurTokens, borderTokens, gradientTokens, textStyleTokens, shadowTokens] = matchedTokens;

  await removeDirectory(outputDir);

  return { colorTokens, blurTokens, borderTokens, gradientTokens, textStyleTokens,shadowTokens };
}
