...
exports.retrieveImageUrls = async(client, file, data) => {
  try {
    ...
    const exportableLayers = data.filter(component => component.exportSettings);

    // Parse all necessary data for the images request to figma api
    const imageRequestData = parseImageRequestData(exportableLayers);
    ...
    return imageRequestResults.reduce((acc, result) => {
      result.urls.forEach(({ id, url }) => {
        const matchingFileData = exportableLayers.find(d => d.id === id);

        const fileDirStructure = matchingFileData.name.split('/');
        const fileName = toSnakeCase(fileDirStructure.pop());
        const fullFileDirString = fileDirStructure.length ? `${fileDirStructure.join('/')}/` : '';

        acc.push({
          id,
          fileUrl: url,
          fileName: `${fullFileDirString}${fileName}${result.suffix}.${result.format}`,
        });
      });
      return acc;
    }, []);
