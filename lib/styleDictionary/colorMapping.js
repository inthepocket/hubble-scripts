...
const mapBorderStyleColors = (borderStyles = []) => {
  const output = createTreeOfLeastDepth(
    borderStyles
      ...
      .map(borderStyle => {
        const color = borderStyle.color;
        color.borderId = borderStyle.id;
        return color;
      }
    ),
    ['borderId'],
...
const mapShadowStyleColors = (shadowStyles = []) => {
  const output = createTreeOfLeastDepth(
    shadowStyles
      ...
      .map(shadowStyle => {
        const color = shadowStyle.color;
        color.shadowId = shadowStyle.id;
        color.shadowType = shadowStyle.type;
        return color;
      }
    ),
    ['shadowId', 'shadowType'],
